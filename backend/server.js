...
app.get('/api/debug', (req, res) => {
    ...
    res.json({ 
        message: 'OmniVest API is running!', 
        timestamp: new Date(),
        port: PORT,
        routes: ['/api/investments', '/api/prices'],
        database: {
            connected: dbStatus,
            type: dbStatus ? 'MongoDB' : 'Fallback Memory Storage',
            status: dbStatus ? 'Production Ready' : 'Demo Mode'
        }
    });
});
